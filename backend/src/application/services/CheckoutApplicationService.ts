...
export class CheckoutApplicationService {
  ...
  async procesarCompra(
    ...
    trace.push({
      ...
      detail: `Subtotal: $${subtotal} USD.`,
    });

    const montoDescuento = this.calculadorDescuentoService.calcularDescuento(
      usuario,
      cupon,
      subtotal,
    );
    ...
    trace.push({
      ...
      detail: `Descuento: $${montoDescuento} USD.`,
    });
    ...
    trace.push({
      ...
      detail: 'Fraude validado.',
    });

    const totalFinalMoneda = await this.conversorMonedaService.convertir(
      totalVentaUsd,
      'USD',
      pagoData.moneda,
    );
    trace.push({
      ...
      method: 'convertir',
      detail: `Total en ${pagoData.moneda}: ${totalFinalMoneda}.`,
    });

    const pagoResultado = this.procesadorPagoService.procesarPago(
      usuario,
      new Pago(pagoData.metodo, totalFinalMoneda, pagoData.moneda),
    );
    trace.push({
      step: 8,
      type: 'DOMAIN_SERVICE',
      ...
      method: 'procesarPago',
      detail: `Pago ${pagoResultado.estado}.`,
    });
