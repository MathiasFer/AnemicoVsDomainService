...
export class InMemoryUsuarioRepository implements IUsuarioRepository {
  ...
  private precargarDatos() {
    ...
    this.usuarios.set(1, new Usuario(1, 'Juan Pérez', 'juan@test.com', 1000, false, 10, 'USD'));
    // 2. Usuario VIP con saldo
    this.usuarios.set(2, new Usuario(2, 'María López (VIP)', 'maria@test.com', 500, true, 15, 'USD'));
    // 3. Usuario sin saldo (para forzar error de saldo insuficiente)
    this.usuarios.set(3, new Usuario(3, 'Carlos Ruiz (Sin Saldo)', 'carlos@test.com', 0, false, 20, 'USD'));
    // 4. Usuario Sospechoso (para forzar error de nivel de riesgo en ValidadorFraudeService)
    this.usuarios.set(4, new Usuario(4, 'Pedro Gómez (Alto Riesgo)', 'pedro@test.com', 2000, false, 90, 'USD'));
  }
